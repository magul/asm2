...
def get_vaccinations_outstanding(dbo):
    ...
    """
    return db.query(dbo, "SELECT av.ID AS vaccid, a.ID AS animalid, a.ShelterCode, " +
        "a.AnimalName, " +
...
def get_date_of_birth(dbo, animalid):
    ...
    """
    return db.query_date("SELECT DateOfBirth FROM animal WHERE ID = %d" % animalid)
...
def get_date_brought_in(dbo, animalid):
    ...
    """
    return db.query_date("SELECT DateBroughtIn FROM animal WHERE ID = %d" % animalid)
...
def update_variable_animal_data(dbo, animalid):
    ...
    """
    db.make_update_sql("animal", "ID = %d" % animalid, (
...
def update_all_variable_animal_data(dbo):
    ...
    animals = db.query("SELECT ID FROM animal")
    for a in animals:
