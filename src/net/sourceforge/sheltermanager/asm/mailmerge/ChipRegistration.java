 ...
 * who have been adopted between two dates
 * and the microchip numbers they have been given
 * etc. To send on to the Chipping company (Eg: Petlog)
 *
 * @author  Robin Rawson-Tetley
 */
public class ChipRegistration extends MailMerge {
    /** MySQL date to show vaccinations for */
    private String theLDate = "";
    private String theUDate = "";

    public ChipRegistration() {
        // Ask how many days in advance they would 
        // like the source for.
        String ldate = Dialog.getDateInput(Global.i18n("mailmerge",
                    "Please_enter_the_start_date_you_would_like_to_prepare_chip_registration_letters_for"),
                Global.i18n("mailmerge", "Chip_Registration_Mail_Merge"));
        String udate = Dialog.getDateInput(Global.i18n("mailmerge",
                    "Please_enter_the_end_date_you_would_like_to_prepare_chip_registration_letters_for"),
                Global.i18n("mailmerge", "Chip_Registration_Mail_Merge"));
    ...
    protected String getFileName() {
        return Global.i18n("mailmerge", "ChipRegistrations.csv");
    }
    ...
    protected void getData() throws CursorEngineException, NoDataException {
        ...
        ad.openRecordset("MovementDate >= '" + theLDate + "'" +
            " AND MovementDate <= '" + theUDate + "' AND MovementType = " +
            Adoption.MOVETYPE_ADOPTION);

        if (ad.getEOF()) {
            throw new NoDataException(Global.i18n("mailmerge",
                    "There_are_no_required_chip_registrations_between_those_dates."));
        }
        ...
        while (!ad.getEOF()) {
            try {
                ...
                theData[row][8] = add[3];
                theData[row][9] = add[4];
                theData[row][10] = own.getOwnerTown();
                theData[row][11] = own.getOwnerCounty();
                ...
                theData[row][23] = Utils.nullToEmptyString(an.getMarkings());
                row++;
