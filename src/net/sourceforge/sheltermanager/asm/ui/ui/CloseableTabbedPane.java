...
public class CloseableTabbedPane extends JTabbedPane implements MouseListener,
    ...
    private void processMouseEvents(MouseEvent e) {
        ...
        if (icon != null) {
            ...
            Point pos = (headerViewport == null) ? new Point()
                                                 : headerViewport.getViewPosition();
    ...
    protected boolean fireCloseTab(int tabIndexToClose) {
        ...
        for (int i = 0; i < listeners.length; i++) {
            if (listeners[i] instanceof CloseableTabbedPaneListener) {
                if (!((CloseableTabbedPaneListener) listeners[i]).closeTab(
                            tabIndexToClose)) {
                    closeit = false;
    ...
    class CloseTabIcon implements Icon {
        ...
        public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            try {
                ...
                int tabNumber = tabbedpane.getUI()
                                          .tabForCoordinate(tabbedpane, x, y);
