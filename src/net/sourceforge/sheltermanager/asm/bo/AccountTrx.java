...
public class AccountTrx extends UserInfoBO<AccountTrx> {
   ...
   public void setSourceAccountID(Integer newValue) throws CursorEngineException {
   ...
   public void setDestinationAccountID(Integer newValue) throws CursorEngineException {
   ...
   public void setOwnerDonationID(Integer newValue) throws CursorEngineException {
     ...
     * @return A Trx object
     */
    public static Trx getTransactionByID(Integer transactionId, Integer accountId) throws Exception {
        AccountTrx t = new AccountTrx();
        t.openRecordset("ID = " + transactionId);
        return new Trx(t, accountId, 0);
    }

    public static void markReconciled(Integer transactionId) throws Exception {
        DBConnection.executeAction("UPDATE accountstrx SET Reconciled = 1 WHERE ID = " + transactionId);
    }
    ...
    public static void saveTransaction(Trx x) throws Exception {
        ...
        if (x.ownerDonationId > 0) {
            DBConnection.executeAction("UPDATE ownerdonation SET Donation = " + t.getAmount().doubleValue() + " WHERE ID = " + x.ownerDonationId);
        }
    ...
    public static class Trx {
        ...
        public Trx(AccountTrx t, Integer thisAccount, double startbalance) throws Exception {
            ...
            if (t.getSourceAccountID().equals(thisAccount)) {
                ...
                otherAccountCode = DBConnection.executeForString("SELECT Code FROM accounts WHERE ID = " + otherAccountId);
                balance -= withdrawal;
            }
            // it's a deposit
            else {
