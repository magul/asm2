...
public class AvidRegistration {
	...
	public void register() {
		...
		try {
			...
			String avidUrl = Configuration.getString("AvidURL", DEFAULT_PETTRAC_URL);
			...
			if (breed.startsWith("Domestic Long")) breed = "DLH";
			if (breed.startsWith("Domestic Short")) breed = "DSH";
			if (breed.startsWith("Domestic Medium")) breed = "DSLH";
			
			String name = movement.getAnimal().getAnimalName();
			String microchip = movement.getAnimal().getMicrochipNumber();
			String implantdate = formatDate(movement.getAnimal().getMicrochipDate());
			...
			String address2 = "";
			String address3 = "";
			if (add.length > 0) address1 = add[0];
			if (add.length > 1) address2 = add[1];
			if (add.length > 2) address3 = add[2];
			...
			String sex = movement.getAnimal().getSexName().substring(0, 1).toUpperCase();
			String neutered = movement.getAnimal().getNeutered().intValue() == 1 ? "true" : "false";
			...
			if (species.startsWith("Dog")) species = "Canine";
			else if (species.startsWith("Cat")) species = "Feline";
			else if (species.startsWith("Bird")) species = "Avian";
			else if (species.startsWith("Horse")) species = "Equine";
			else if (species.startsWith("Reptile")) species = "Reptilian";
			else species = "Other";
			...
			if (!microchip.startsWith("977")) {
				Dialog.showError("Valid AVID microchips start with the prefix '977'");
				return;
			}
			...
			if (orgserial.equals("") || orgpassword.equals("")) {
				Dialog.showError("You need to set your AVID organisation info under System Options.");
				return;
			}
			...
			h.append("<p align=\"center\"><input type=\"submit\" value=\"Submit Registration\" /></p>");
	...
	private void addHidden(String name, String value) {
		h.append("<input type=\"hidden\" name=\"" + name + "\" value=\"" + value + "\" />\n");
	}
